...
class AIGenTextDetector:
    ...
    def __init__(self, token):
        ...
        self.class_max = [10, 40, 60, 80, 90]
        
        # openai suggested thresholds below
        # self.class_max = [10, 45, 90, 98, 99]

    def predict(self, text, all_probs=False):
        data = {
            'prompt': text + "».\n<|disc_score|>",
            'max_tokens': 1,
            'temperature': 1,
...
if __name__ == "__main__":
    ...
    token = "Bearer sess.." # Token hidden for privacy
    review = "Your Review here"
    all_probs = False # Change to True if requiring all probabilities default is False
    ai_text_detect = AIGenTextDetector(token)
    response = ai_text_detect.detect(review, all_probs)
    
    print(responses) # Change based on backend integration
